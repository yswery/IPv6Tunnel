...
class IpUtils
{
    ...
    public function cidr2range($cidr, $returnInDecimal = false)
    {
        $addressParts = explode('/', $cidr);
        $startIp      = $addressParts[0];
        $cidrSize     = $addressParts[1];

        if ($this->getInputType($startIp) === 6) {
            $prefixSizes = $this->IPv6cidrIpCount();
        } else {
            $prefixSizes = $this->IPv4cidrIpCount();
        }

        // Check if its a valid prefix size
        if (isset($prefixSizes[$cidrSize]) !== true) {
            return false;
        }
